...
export default function BenefitsConfirmPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      router.push(`/member/claims/benefits/submitted?reference=${encodeURIComponent(data.reference)}&status=${data.status}`)
  ...
  return (
    <div className="max-w-xl mx-auto space-y-8">
      <StepIndicator steps={[...BENEFITS_STEPS]} currentStep={11} />
      ...
      <Button onClick={submit} disabled={loading || otp.length !== 6} className="w-full bg-[#7C3AED] text-white">{loading ? "Submitting…" : "Submit Claim"}</Button>
    </div>
  )
}
